...
#   -Pause           wait for Enter before closing (useful when double-clicking)
#
...
# Branch to download from when no local copy is found. Windows support lives on
# Windows_Server until merged - downloading `main` would silently install a Unix-only server.
$RepoBranch = if ($env:OPENCLAW_IOS_BRANCH) { $env:OPENCLAW_IOS_BRANCH } else { 'Windows_Server' }
$RepoRaw = "https://raw.githubusercontent.com/FreQRiDeR/OpenClaw-ios/$RepoBranch"
...
function Die($m)  {
    Bad $m
    Write-Host "  Full installer log: $script:TranscriptPath" -ForegroundColor Yellow
    if ($Pause) { Read-Host 'Press Enter to close' | Out-Null }
    exit 1
}
function Step($m) { Write-Host "`n== $m" }

# Always retain output, including failures from child scripts.
$script:TranscriptPath = Join-Path $env:TEMP 'openclaw-ios-install.log'
try { Start-Transcript -Path $script:TranscriptPath -Force | Out-Null } catch { }
Write-Host "Installer log: $script:TranscriptPath"
...
# External `tailscale serve --bg` connects through the ordinary loopback gateway listener.
# OpenClaw 8.x requires the immediate proxy to be trusted while bearer auth stays enabled.
$trusted = @($cfg.gateway.trustedProxies)
if ($trusted -contains '127.0.0.1') {
    Ok 'gateway.trustedProxies narrowly trusts the IPv4 loopback proxy'
} else {
    Warn 'External Tailscale proxy will get HTTP 403 proxy_attribution_required.'
    Write-Host '       The installed configure_gateway_proxy.ps1 helper safely fixes this after extraction.'
}
if ($cfg.gateway.auth.mode -eq 'token') { Ok 'gateway.auth.mode = token' }
else { Warn 'Set: openclaw config set gateway.auth.mode token' }
if ($cfg.gateway.auth.allowTailscale -eq $false) { Ok 'gateway.auth.allowTailscale = false' }
else { Warn 'Set: openclaw config set gateway.auth.allowTailscale false --strict-json' }
if ($cfg.gateway.tailscale.mode -eq 'off') { Ok 'gateway.tailscale.mode = off (external Serve is sole owner)' }
else { Warn 'Set: openclaw config set gateway.tailscale.mode off' }
...
# The install destination is never used as a source (it would be deleted before copying).
$SrcDir = $null; $SrcZip = $null; $TmpDl = $null
$DestFull = [IO.Path]::GetFullPath($Dest).TrimEnd('\')
if ($ScriptDir) {
    $cand = Join-Path $ScriptDir 'openclaw-stats-server'
    if ((Test-Path (Join-Path $cand 'scripts\dashboard\stats_server.py')) -and ([IO.Path]::GetFullPath($cand).TrimEnd('\') -ne $DestFull)) {
        $SrcDir = $cand; Ok "using repo checkout: $SrcDir"
    } else {
        foreach ($z in @((Join-Path $ScriptDir 'docs\openclaw-stats-server.zip'), (Join-Path $ScriptDir 'openclaw-stats-server.zip'))) {
            if (Test-Path $z) { $SrcZip = $z; Ok "using local zip: $SrcZip"; break }
        }
    }
}
if (-not $SrcDir -and -not $SrcZip) {
    ...
    Write-Host "  no local copy found - downloading $RepoRaw/docs/openclaw-stats-server.zip ..."
    ...
    Ok ("downloaded {0:N0} KB from branch '$RepoBranch'" -f ((Get-Item $SrcZip).Length / 1KB))
}
...
# Refuse to install a copy that lacks Windows support (e.g. an old zip or the wrong branch)
foreach ($req in @('scripts\dashboard\ensure_stats_server.ps1', 'scripts\setup_tailscale.ps1', 'scripts\configure_gateway_proxy.ps1')) {
    if (-not (Test-Path (Join-Path $Dest $req))) {
        Remove-Item $Dest -Recurse -Force -ErrorAction SilentlyContinue
        Die "installed copy is missing $req - this zip predates Windows support. Re-zip from the Windows_Server branch (or set OPENCLAW_IOS_BRANCH)."
    }
}
if (-not (Select-String -Path (Join-Path $Dest 'scripts\dashboard\system_stats.py') -Pattern 'IS_WIN' -Quiet)) {
    Remove-Item $Dest -Recurse -Force -ErrorAction SilentlyContinue
    Die 'system_stats.py has no Windows branch - this zip predates Windows support.'
}
Ok 'Windows support verified in installed files'
...
if ($DoTailscale) {
    ...
    $tailArgs = @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $Dest 'scripts\setup_tailscale.ps1'))
    if ($Pause) { $tailArgs += '-Pause' }
    & powershell @tailArgs
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  Full installer log: $script:TranscriptPath" -ForegroundColor Yellow
        try { Stop-Transcript | Out-Null } catch { }
...
Write-Host @"
  ...
  Full installer log: $script:TranscriptPath
"@
try { Stop-Transcript | Out-Null } catch { }
if ($Pause) { Read-Host 'Press Enter to close' | Out-Null }
